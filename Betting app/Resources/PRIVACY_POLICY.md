...
When you create a Pick1 account we collect:
...
- Browsing or search history outside of Pick1
...
- **Improve Pick1** — anonymized usage patterns help us decide which features to build next (we don't tie this to your identity in any analysis)
...
We share information with **only** the third parties needed to deliver Pick1:
...
Pick1 data is hosted in EU (Supabase EU region) and US (Apple's payment processing). If you access Pick1 from outside these regions, your data may be transferred to and processed in regions other than your own. We rely on Standard Contractual Clauses to govern these transfers in compliance with GDPR.
...
We may update this policy periodically. When we do we'll bump the "Effective date" at the top and, for material changes, send you a notice via email or an in-app banner before the change takes effect. Continued use of Pick1 after a policy update constitutes acceptance of the revised policy.
